...
## Activity 2.5: Production Serving Layer

### Objective
Wrap the optimized vLLM server in a production-grade REST API layer with authentication, rate limiting, structured logging, process persistence via systemd, and a deployment path for persistent hosting. This is what makes the LoraForge output actually usable as a live endpoint, not just a benchmark artifact.

### Why It Exists
Activities 1 and 2 deploy vLLM on a Lambda instance and immediately tear it down after benchmarking. There is no auth, no external access, no process manager. Before Activities 3–4 run KV cache and speculative decoding optimizations, having a real serving layer means you can test those optimizations against a live endpoint under real HTTP traffic — not just a subprocess you spin up and kill. It also makes the project demonstrable in interviews and to recruiters: "I have a live API endpoint you can query" is categorically more impressive than "I have benchmark numbers."

### What You Build

- **`api_server.py`**: FastAPI wrapper that proxies to vLLM, adds auth/rate-limiting middleware, structured JSON request logging, and a Prometheus-compatible `/metrics` endpoint
- **`middleware/auth.py`**: X-API-Key header validation against a config-defined key list
- **`middleware/rate_limiter.py`**: Per-API-key token bucket rate limiter (configurable requests/min)
- **`middleware/request_logger.py`**: Logs each request with latency, model name, token counts, and error status to a JSON log file
- **`systemd/loraforge.service`**: systemd unit file — auto-starts vLLM + FastAPI on boot, auto-restarts on crash
- **`nginx/loraforge.conf`**: Nginx reverse proxy — TLS termination (Let's Encrypt), connection pooling, request buffering
- **`deploy/modal_deploy.py`**: Modal serverless deployment — zero idle cost, ~10s cold start, HTTP endpoint that scales to zero when unused
...
Nginx (TLS, buffering)
     │ HTTP :8001 (internal)
     ▼
FastAPI wrapper (auth + rate-limit + logging)
     │ HTTP :8000 (loopback)
     ▼
vLLM server (quantized model from Activity 2)
     │
NVIDIA GPU (INT4 AWQ or FP8 weights)
```

### Inputs
- Quantized model checkpoint from Activity 2 (INT4 AWQ or FP8)
- `config.yaml` with API keys, rate limits, vLLM path, and ports
- SSL certificate (Let's Encrypt via certbot, or self-signed for internal use)

### Outputs
- Live HTTPS endpoint: `POST https://<your-domain>/v1/chat/completions`
- `serving_logs/` — one JSON file per day with all request logs
- systemd service running on the Lambda instance

### GPU / Hosting Cost

| Hosting option | Cost | Idle cost | Best for |
|---|---|---|---|
| Lambda A10G (on-demand) | $0.75/hr | Full rate | Short-lived demos |
| RunPod A10 (reserved) | ~$0.44/hr | Full rate | Sustained testing |
| Modal serverless | ~$0.001/req | $0 | Low-traffic public demo |
| HuggingFace Endpoints | ~$1.70/hr (A10G) | Full rate | Public-facing production |

### Key Concepts Learned
- How to sit a FastAPI app in front of a subprocess server (proxy pattern)
- systemd unit files: ExecStart, ExecStop, Restart=on-failure, EnvironmentFile
- Nginx as a reverse proxy: `proxy_pass`, `proxy_read_timeout`, connection pooling
- Token bucket rate limiting: how it prevents API abuse without impacting burst traffic
- Modal's serverless GPU model: container image, `@modal.endpoint`, cold start mechanics

### Placement in the Activity Sequence

Activity 2.5 is optional but strongly recommended for interview narrative. It sits after Activity 2 (quantized model is available) and before Activity 3 (you benchmark KV cache against the live endpoint). The sequence becomes:

```
Activity 2 → INT4 AWQ checkpoint
Activity 2.5 → checkpoint is live behind a real HTTPS endpoint
Activity 3 → benchmark prefix caching against the live endpoint
Activity 4 → benchmark speculative decoding against the live endpoint
Activity 5 → dashboard shows all results, loraforge serve CLI wraps everything
```
...
| Phase | Instance | Cost |
|---|---|---|
| FastAPI + nginx setup | No GPU needed (dev on Mac) | $0 |
| Test run on Lambda | A10G @ $0.75/hr × 2hrs | $1.50 |
| Persistent demo endpoint | RunPod A10 × 7 days | ~$74 |
| Modal demo endpoint | Pay-per-use | < $1 total |
...
activity25_serving/
├── config.yaml
├── main.py                  # Entry point: starts FastAPI wrapper + waits for vLLM ready
├── api_server.py            # FastAPI app with all routes and middleware
├── middleware/
│   ├── auth.py
│   ├── rate_limiter.py
│   └── request_logger.py
...
└── deploy/
    ├── modal_deploy.py
    └── setup_serving.sh     # Installs nginx, certbot, sets up systemd service
```

### Run Commands

```bash
# Local test (no GPU needed — point at any running vLLM)
python -m activity25_serving.main --vllm-url http://localhost:8000 --port 8001

# Send an authenticated request
curl -X POST http://localhost:8001/v1/chat/completions \
  -H "X-API-Key: your-key" \
  -H "Content-Type: application/json" \
  -d '{"model":"cybersec-analyst-fp16","messages":[{"role":"user","content":"What is Log4Shell?"}],"stream":true}'

# Deploy to Modal (serverless, zero idle cost)
modal deploy activity25_serving/deploy/modal_deploy.py
```
